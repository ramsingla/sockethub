  ...
  pub.post = function (job) {
    ...
    try {
      session.getConfig('credentials').then(function (creds) {
        ...
        if (typeof twit === 'string') {
          q.reject(twit);
        } else if (job.object.retweet) {
          console.log('statuses/retweet/:id', { id: job.object.retweet });
          twit.post('statuses/retweet/:id', { id: job.object.retweet }, function(err, reply) {
            if (err) {
              console.debug('Retweeting failed: ', err.data);
              var errmsg = '';
              if (err.data) {
                errmsg = err.data;
          ...
          console.log('statuses/update', { status: job.object.text, in_reply_to_status_id: job.object.in_reply_to_status_id_str });
          twit.post('statuses/update', { status: job.object.text, in_reply_to_status_id: job.object.in_reply_to_status_id_str }, function(err, reply) {
            if (err) {
